...
- [Edit the package manifest](#edit-the-package-manifest)
...
Replace `My new repo description` by a short project description
...
Select the template repository of the ggsuite organization

Enter `my-new-repo` as `name`

Into the `Description` field, paste `My new repo description`

Create the repo as usual

## Setup branch rules

Open the settings of your new repository:
...
- [x] `Require status checks to pass`
  ...
  - Select the `Build and Test` GitHub Actions
...
```bash
git clone git@github.com:ggsuite/my-new-repo.git
cd my-new-repo
```

Install the dependencies and run the build with the tooling of your
ecosystem (e.g. `pnpm install && pnpm build` or `dart pub get`).
...
```bash
node scripts/rename_class.js template-project my-new-repo
```

## Edit the package manifest

Open the package manifest (`package.json` or `pubspec.yaml`) and apply the
following changes:

Reset the version to `0.0.1`

Set the description to `My new repo description`
