...
In Vscode, always quick open files by pressing `Cmd+P`, then typing fragments
of the file path or name, and pressing `Enter`.

> Tip: You can also use `Cmd+Shift+P` to open the Command Palette, but
> `Cmd+P` is faster for files.
...
> Note: By default, search is not case-sensitive.
