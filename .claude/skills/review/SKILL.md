...
- **DNA test**: projects using `gg_dna` run a placed DNA test (e.g.
  ...
  - "DNA instances updated — review & commit": review the written
    updates, then propose committing them.
  - "instances were modified by hand": the fix is to move the change
    into the DNA layer that owns the file (or `dna/` of a `role: dna`
    repo), never to adjust the instance.
  - "would overwrite uncommitted work": commit or stash the listed
    files, then rerun.
